...
// Função de criptografia fornecida para o desenvolvimento
char* encrypt(const char* str) {
    ...
    str_result[TAMANHO_SENHA] = '\0'; // Termina o string
    return str_result;
}
...
void quebrar_senhas(const char* senha_criptografada, const char* arquivo_saida) {
    ...
    FILE* file_out = fopen(arquivo_saida, "a"); // Abrir o arquivo
    if (!file_out) {
...
void* processa_arquivo(void* args) {
    ...
    // A função de leitura sera implementada para o arquivo de quebra de senha
    ...
    fscanf(file_in, "%s", senha_criptografada); // Leitura da senha criptografada
...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(argv[1], "-p") == 0) {
        ...
        // Processo Principal espera o processo filho
        while (wait(NULL) > 0);
